...
창업자는 전 FTX COO 컨스탄스 왕(Constance Wang). FTX 이후 이름을 버리지 않고 새로 시작했다는 점에서 커뮤니티 신뢰를 얻었다. VC 투자 없이 자체 자금으로 운영 중이다.

2025년 9월 이후 솔라나 퍼프 DEX 중 누적 거래량 1위를 유지 중이며, 2026년 초 기준 TVL 4천만 달러를 넘었다.

포인트 시스템이 운영 중이다. 매주 목요일 50만 포인트를 배분하며, 시즌 1이 완료되고 현재 시즌 2(Boost Windows)가 진행 중이다. 워시 트레이딩이 탐지되면 소급 삭감하는 강력한 시빌 정책을 명시하고 있다.
...
토큰 티커는 DNG, 총 공급량 3만 개로 고정되어 있다. 수수료는 USDC로 수납해 DNG를 매입 후 소각하는 디플레이션 구조다.

시드 투자는 Lemniscap, Hack VC, Delphi Labs가 참여했으며 총 360만 달러 규모다.

2026년 4월 기준 내부 테스트넷 단계이고 퍼블릭 데모넷은 단계적으로 공개 중이다. 공식 에어드랍 발표는 없다. 다만 초기 단계에 테스트넷 참여 기록을 쌓아두는 것이 나중을 대비하는 일반적인 방법이다.
...
### Based — HyperEVM (2025년)

HyperEVM 위에서 운영되는 트레이딩 플랫폼이다. Hyperliquid 생태계 성장을 목표로 하며, Based 트레이더들이 만들어내는 거래량이 HyperEVM 생태계에서 상당한 비중을 차지하고 있다.

플랫폼 사용자들을 대상으로 에어드랍을 실시했으며, XP 프로그램을 통해 포인트를 적립하면 추가 보상을 받는 구조다.

HyperEVM에서 직접 거래하고 활동하는 사람에게 보상이 돌아가는 방식이라, 별도로 무언가를 하기보다 HyperEVM을 자연스럽게 쓰는 것이 참여 전략이다.
