...
- **2025년 Q1**: Demonet-1 공개. 계정 시스템, 오라클, 기본 스팟 DEX 테스트
- **2025년 Q3**: Chaosnet 런칭. 실험적 운영 시작
- **2025년 5월**: Galxe Starboard 참여. 총 공급량의 1%를 커뮤니티 리워드로 배분 시작, 테스트넷 OAT 배포
- **2026년 1월 6일**: **메인넷 알파 공개**. ETH/USDC 스팟 거래부터 시작
...
- 블록 타임: **0.5초**
- TPS: **약 2,000**
- 합의 방식: Tendermint 기반, 20개 <abbr title="검증자가 직접 신원을 밝히고 권한을 부여받는 방식. PoS처럼 토큰 스테이킹이 아닌 신뢰 기반 운영">PoA 검증자</abbr>
- 즉시 완결성 (파이널리티)
...
대신 트위터 계정 만들듯이 **유저네임을 고른다**. 예: `@larry`. 이 유저네임이 온체인 신원이 된다.

그 아래에 <abbr title="패스키. 기기의 생체인증이나 PIN으로 로그인하는 방식. 비밀번호 없이 지문/얼굴 인식으로 인증">Passkey</abbr>, 기존 MetaMask 지갑, 하드웨어 키 등을 연결할 수 있다. DeFi를 처음 접하는 사람도 진입 장벽이 확 낮아진다.
...
이 수수료는 DNG를 매입해서 소각(burn)하는 데 쓰인다. DNG의 공급이 줄어드는 디플레이션 구조다.
...
- 참여: Delphi Ventures, Cherry Crypto, Interop, Public Works

Hack VC는 Sui, Aptos, Solana 생태계 투자로 알려진 곳이다.

Delphi Ventures는 DeFi 분석으로 업계에서 신뢰받는 리서치+투자사다.

단순한 이름값 채우기 투자자들이 아니다.

익명 팀이 아니다. Hack VC 같은 실명 기관이 투자한 프로젝트다.
...
DNG는 아직 공개 시장에 상장되지 않은 미발행 토큰이다. TGE는 **2026년 여름**으로 예정되어 있다.
...
- **ICO 30%** (9,000개): MiCA 프레임워크 준수하에 EU 거주자 대상 판매
...
루트박스 안에 뭐가 들어있는지는 아직 공개되지 않았다. 추가 포인트, 특별 배지, 또는 다른 리워드일 가능성이 있다.

단 거래량 기반이라 볼트 예치만으로는 루트박스가 쌓이지 않는다.

### 전략 4. OAT 연동 — 이미 있다면 지금 바로
